def max_path_sum(triangle):
    ...
    for i in range(1, len(triangle)):
        for j, k in enumerate(triangle[i]):
            ...
            print("triangle[{i}][{j}] = {k} + max([triangle[{i}-1][{j}], triangle[{i}-1][{j}+1]])".format(i = i,j = j,k = k))
            print("triangle[{i}][{j}] = {k} ".format(i=i, j=j, k=(k + max([triangle[i-1][j], triangle[i-1][j+1]]))))
    return triangle[-1][0]
